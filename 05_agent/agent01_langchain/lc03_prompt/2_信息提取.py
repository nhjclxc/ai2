...
for example in examples_data:
    ...
    answer_content = json.dumps(example["answers"], ensure_ascii=False)
    messages.append({"role": "assistant", "content": answer_content})

for question in questions:
    msgs = messages + [{"role": "user", "content": f"请根据上述示例，来抽取这个内容的响应结构数据，内容为：{question}"}]
    json_str = json.dumps(msgs, ensure_ascii=False)
    print(json_str)
